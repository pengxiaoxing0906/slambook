...
void find_feature_matches(const Mat &img1,const Mat&img2,
                          ...
                          std::vector<DMatch>&matches);
Point2d pixel2cam(const Point2d&p);//像素坐标转相机归一化坐标
...
int main(int argc,char**argv)
{
    ...
    {
        Point2d pt1=pixel2cam(keypoints_1[m.queryIdx].pt);//像素坐标转换为相机归一化坐标
    ...
    for(int i=0;i<matches.size();i++)
    {
        ...
        //第2张图
        Point2d  pt2_cam = pixel2cam(keypoints_1[matches[i].trainIdx].pt);
...
Point2d pixel2cam(const Point2d&p) //像素坐标转相机归一化坐标
{
   ...
   // Mat K=(Mat_<double>(3,3)<<520.9,0,325.1,0,521.0,249.7,0,0,1);
    ...
    double cx=325.1;
    double cy=249.7;
   // cout<<((u-cx)/fx)<<" "<<((v-cy)/fy)<<endl;
    return  Point2d(((u-cx)/fx),((v-cy)/fy)
    );
}
