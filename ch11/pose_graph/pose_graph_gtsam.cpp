...
int main(int argc,char**argv)
{
    ...
    //写入g2o文件，伪装成g2o中的边和顶点，以便用g2o_viewer查看
    ...
    for(gtsam::NonlinearFactor::shared_ptr factor:*graph)
    {
        ...
        if(f)
        {
            ...
            gtsam::noiseModel::Gaussian::shared_ptr guassianModel=dynamic_pointer_cast<gtsam::noiseModel::Gaussian>(model);
            if(guassianModel)
            {
                //write the edge information
                gtsam::Matrix info=guassianModel->R().transpose()*guassianModel->R();
                ...
                infoG2o.block(0,3,3,3)=info.block(3,0,3,3);
                for(int i=0;i<6;i++)
                {
                    for(int j=i;j<6;j++)
                    {
