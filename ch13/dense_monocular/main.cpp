...
inline double getBilinearInterpolatedValue(const Mat&img,const Vector2d&pt)
{
    ...
    double xx=pt(0,0)-floor(pt(0,0));//floor()函数功能：向下取整
    double yy=pt(1,0)-floor(pt(1,0));
    return (
            ((1-xx)*(1-yy)*double(d[0])+xx*(1-yy)*double(d[1])+(1-xx)*yy*double(d[img.step])+xx*yy*double(d[img.step+1]))/255.0
            );
...
//检测一个点是否在图像边框内　pt(0,0)指x，为此像素点的width，pt(1,0)指y,为此像素点的height
...
void showEpipolarMatch(const Mat&ref,const Mat&curr,const Vector2d&px_ref, const Vector2d&px_curr);
 ...
 //显示极线
 void showEpiploaarLine(const Mat&ref, const Mat&curr,const Vector2d&px_ref,const Vector2d&px_min_curr,const Vector2d& px_max_curr);
...
int main(int argc,char**argv)
{
    ...
    vector<SE3>poses_TWC; //Twc表示从相机坐标系到世界坐标系的变换矩阵　书上P60
    ...
    //第一张图
    Mat ref=imread(color_image_files[0],0);//以灰度图的形式读入
    SE3 pose_ref_TWC=poses_TWC[0];
    double init_depth=3.0; //深度初始值
    double init_cov2=3.0; //方差初始值
    Mat depth(height,width,CV_64F,init_depth); //深度图  Mat(int rows, int cols, int type, const Scalar& s);
...
bool readDatasetFiles(const string& path,vector<string>&color_image_files,vector<SE3>&poses)
{
    ...
    while(!fin.eof())
    {
        //数据格式:图像文件名tx,ty,tz,qx,qy,qz,qw TWC
        string image;
        fin>>image;
        double data[7];
        for(double& d:data)
...
bool update(const Mat&ref, const Mat&curr,const SE3&T_C_R,Mat &depth,Mat&depth_cov)
{
    //cout<<"走到这里了吗？update"<<endl;
#pragma omp parallel for
    for(int x=boarder;x<width-boarder;x++)  //遍历当前帧的每一个像素块
#pragma omp parallel for
...
bool epipolarSearch(const Mat&ref,const Mat&curr,const SE3&T_C_R,const Vector2d&pt_ref,const double&depth_mu,const double&depth_cov,Vector2d&pt_curr)
{
   // cout<<"走到这里了吗？epipolarSearch"<<endl;
    Vector3d f_ref=px2cam(pt_ref);
    f_ref.normalize();
    Vector3d P_ref=f_ref*depth_mu; //参考帧的Ｐ向量

    Vector2d px_mean_curr=cam2px(T_C_R*P_ref);  //按深度均值投影的像素 SE3类型的维度
    double d_min=depth_mu-3*depth_cov,d_max=depth_mu+3*depth_cov;
    if(d_min<0.1)d_min=0.1;
    Vector2d px_min_curr=cam2px(T_C_R*(f_ref*d_min)); //按最小深度投影的像素
    Vector2d px_max_curr=cam2px(T_C_R*(f_ref)*d_max);//按最大深度投影的像素

    Vector2d epipolar_line=px_max_curr-px_min_curr; //极线
    Vector2d epipolar_direction=epipolar_line;  //极线方向
    epipolar_direction.normalize();
    double half_length=0.5*epipolar_line.norm(); //极线线段的半宽度
    if(half_length>100)half_length=100;

    //showEpiploaarLine(ref,curr,pt_ref,px_min_curr,px_max_curr);

    //在极线上搜索，以深度均值点为中心，左右各取半宽度
    double best_ncc=-1.0;
...
bool updateDepthFilter(const Vector2d&pt_ref,const Vector2d&pt_curr,const SE3&T_C_R,Mat&depth,Mat& depth_cov)
{
    ...
    Vector3d xn=lambdavec(1,0)*f2;
    Vector3d d_esti=(xm+xn)/2.0; //三角化计算得的深度
    ...
    double a_norm=a.norm(); //这个函数也是归一化的意思吗？
    double alpha=acos(f_ref.dot(t)/t_norm);
    double beta=acos(-a.dot(t)/(a_norm*t_norm));
    double beta_prime=beta+atan(1/fx);
    double gama=M_PI-alpha-beta_prime;
    double p_prime=t_norm*sin(beta_prime)/sin(gama);
...
void showEpiploaarLine(const Mat&ref, const Mat&curr,const Vector2d&px_ref,const Vector2d&px_min_curr,const Vector2d& px_max_curr)
{
    ...
    cv::circle(ref_show,cv::Point2f(px_ref(0,0),px_ref(1,0)),5,cv::Scalar(0,250,0),2);
    cv::circle(curr_show,cv::Point2f(px_min_curr(0,0),px_min_curr(1,0)),5,cv::Scalar(0,250,0),2);
    cv::circle(curr_show,cv::Point2f(px_max_curr(0,0),px_max_curr(1,0)),5,cv::Scalar(0,250,0),2);
    cv::line(curr_show,Point2f(px_min_curr(0,0),px_min_curr(1,0)),Point2f(px_max_curr(0,0),px_max_curr(1,0)),cv::Scalar(0,250,0),1);
    imshow("ref",ref_show);
